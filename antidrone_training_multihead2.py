...
def get_used_parameters(model, inputs):
    ...
    def forward_hook(module, input, output):
        # Add the module's parameters to the used_params set
    ...
    # Calculate the number of used parameters
    used_param_count = sum(p.numel() for p in used_params)
    return used_param_count

input1 = torch.randn(1, 4, 64, 64)  # 示例输入2：1个3通道的32x32图像
input2 = torch.randn(1,3, 256,256)  # 示例输入1：1个3通道的32x32图像


# 使用 torchprofile 计算双输入模型的 FLOPs 和参数量
flops = torchprofile.profile_macs(model, (input1, input2))  # 传入元组作为输入
params = sum(p.numel() for p in model.parameters())  # 计算参数量
...
used_params = get_used_parameters(model, (input1, input2))

print(f"Used Parameters: {used_params/1000000}")
